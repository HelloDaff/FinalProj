...
def mentor():
    ...
    try:
        if token_receive:
            ...
            return render_template("MyMentor.html", mentors=mentors,user_info=user_info)
...
def mentor_kami():
    ...
    # Save mentor data to the database
            ...
            mentor_file.save(os.path.join(app.config['UPLOAD_FOLDER'], mentor_file.filename))
...
def admin_home(username):
    ...
    contacts = db.hubungi.find().skip((page - 1) * contacts_per_page).limit(contacts_per_page)
    total_contacts = db.hubungi.count_documents({})
    if user_info is not None:
        return render_template("administrator/dashboardAdmin.html", status=status,user_data=user_data,user_info=user_info, is_admin=True, contacts=contacts, page=page, total_contacts=total_contacts, contacts_per_page=contacts_per_page)
    else:
...
def admin_hubungi(username):
    ...
    contacts_per_page = 10
    contacts = db.hubungi.find().skip((page - 1) * contacts_per_page).limit(contacts_per_page)
    return render_template("administrator/dashboardAdmin.html", status=status,user_data=user_data,contacts=contacts, page=page, total_contacts=total_contacts,user_info=user_info,contacts_per_page=contacts_per_page)
...
def user(username):
    ...
    try:
        ...
        user_info = db.user.find_one({'username': payload['id']})
        user_data = db.user.find_one({"username": username}, {"_id": False})
        return render_template("profile_mem.html", user_info=user_info,user_data=user_data,status=status)
    except (jwt.ExpiredSignatureError, jwt.exceptions.DecodeError):
        return redirect(url_for("home"))
...
def admin(username):
    ...
    try:
        ...
        user_info = db.user.find_one({'username': payload['id']})
        user_data = db.user.find_one({"username": username}, {"_id": False})
        return render_template("administrator/profile.html", is_admin=True,user_info=user_info,user_data=user_data,status=status)
    except (jwt.ExpiredSignatureError, jwt.exceptions.DecodeError):
        return redirect(url_for("home"))
...
def fetch_notifications():
    try:
        ...
        return jsonify({'contact_count': contact_count, 'new_user_count': new_user_count})
...
def get_notifications():
    try:
        ...
        new_users = list(db.user.find({"role": "member"}).sort("timestamp", -1).limit(5))
